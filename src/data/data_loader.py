...
def load_data(file_path: Optional[str] = None) -> pd.DataFrame:
    ...
    if not path:
        # Redundant check as RAW_DATA_PATH is defined, but good practice
    ...
    if not path.exists():
        logger.error(
            f"File not found at: {path}. " "Ensure 'rides_raw.csv' is in 'data/raw'."
        )
    ...
    try:
        df = pd.read_csv(path)
        logger.info(f"Loaded raw data with shape: {df.shape}")  # noqa: E501
        return df
